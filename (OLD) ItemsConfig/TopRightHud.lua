...
local function TopRightHud(props)
	...
	local moneyText = "Cash💵: " .. (money and Alyanum.new(money):toString() or "...")
	local rateText = "\n: " .. ((Alyanum.new(totalMult * rate):toString() .. "/s") or "...")

	if totalMult > 1 and rate then
		rateText = rateText
			.. string.format(
				"\n" .. [[<font color="#FFD700" size="12">(%s × %.2fx)</font>]],
		...
		for multId, multData in pairs(props.multipliers) do
			...
			if expire > 1 or expire == math.huge then
				...
				multipliersText = multipliersText
					.. string.format(
						"\n" .. [[<font color="#00FF88">%.2fx (%02d:%02d)</font>]],
						value,
						minutes,
						seconds
					)
				...
				multipliersText = multipliersText
					.. string.format("\n" .. [[<font color="#00FF88">%.2fx (permanent)</font>]], value)
			end
		end
	end
	-- warn("HUD rate", rate)
